...
let openWindow file = 
    boxChart.Image <- drawChart file
    btnSave.Enabled <- true
    Application.Run mainForm

let saveWindow file2 = 
    boxChart.Image.Save file2
...
let main argv = 
    if( argv.Length > 0)
    then 
        Array.iter(fun x -> openWindow x) argv
    else
        ...
        while looping do 
             Console.Write("Podaj sciezke do zapisu np. ( c:\Users\Łukasz\Nazwa.png ): ") 
             let file2 = Console.ReadLine()
             fille <- file2
             if file2.Length > 0 then
                 looping <- false
        let file2 = fille
        saveWindow file2
